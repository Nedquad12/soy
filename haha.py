...
class StockVolumeMonitor:
    def __init__(self, bot_token: str, admin_ids: List[int] = None):
        ...
        self.monitoring_interval = 90  # 1 1/2 menit
...
🎯 Threshold Alert: {self.volume_threshold}x lipat
    ...
    def calculate_average_volume(self, symbol: str, now: datetime) -> Optional[float]:
        """Hitung rata-rata volume sesuai periode"""
        try:
           ticker = yf.Ticker(symbol)
           if now.hour < 10:
            # Ambil data hari kemarin 09:00–16:00
            ...
            logger.error(f"Error getting reference volume for {symbol}: {e}")
            return None

    
    def should_alert(self, symbol: str, current_volume: float, now: datetime) -> bool:
        """Cek apakah harus kirim alert berdasarkan rata-rata periode"""
        ref_volume = self.calculate_average_volume(symbol, now)
        if ref_volume is None or ref_volume == 0:
            return False
        ratio = current_volume / ref_volume
        return ratio >= 1.7  # Alert jika naik 70%
            ...
            try:
                ...
                for symbol in self.popular_stocks:
                    try:
                        ...
                        if self.should_alert(symbol, data['volume'], current_time):
